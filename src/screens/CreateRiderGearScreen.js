...
export default function CreateRiderGearScreen({ navigation, route }) {
  ...
  const handleSubmit = async () => {
    if (!title.trim()) {
      Alert.alert(t('error'), isArabic ? 'يرجى إدخال اسم المنتج.' : 'Please enter title.');
      return;
    }
    ...
    try {
      const payload = {
        ...
        image_urls: ['https://via.placeholder.com/600x400?text=Rider+Gear'],
      };

      await createRiderGear(payload);
      Alert.alert(
        t('success'),
        isArabic
          ? 'تم إنشاء إعلان مستلزمات الفارس بنجاح، وهو قيد المراجعة.'
          : 'Rider gear listing created successfully and is under review.',
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>
            {isArabic ? 'إضافة مستلزمات فارس جديدة' : 'Add Rider Gear Listing'}
          </Text>
        </View>

        {/* Title */}
        <View style={styles.inputGroup}>
          <Text style={styles.label}>{isArabic ? 'اسم الإعلان / المنتج *' : 'Listing Title *'}</Text>
          <TextInput
            style={[styles.input, { textAlign: isArabic ? 'right' : 'left' }]}
            placeholder={isArabic ? 'مثال: خوذة ركوب خيل حماية عالية' : 'e.g. Premium Riding Helmet'}
            value={title}
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            style={[styles.input, { textAlign: isArabic ? 'right' : 'left' }]}
            placeholder={isArabic ? 'مثال: Samshield, Ariat, Pikeur' : 'e.g. Samshield, Ariat, Pikeur'}
            value={brand}
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>{isArabic ? 'الفئة المستهدفة (الجنس)' : 'Target Gender'}</Text>
          <View style={styles.genderRow}>
            <TouchableOpacity
              style={[styles.genderChip, gender === 'female' && styles.selectedGenderChip]}
              onPress={() => setGender('female')}
            >
              <Text style={[styles.genderChipText, gender === 'female' && styles.selectedGenderText]}>
                {isArabic ? 'نسائي' : 'Women'}
              </Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.genderChip, gender === 'male' && styles.selectedGenderChip]}
              onPress={() => setGender('male')}
            >
              <Text style={[styles.genderChipText, gender === 'male' && styles.selectedGenderText]}>
                {isArabic ? 'رجالي' : 'Men'}
              </Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.genderChip, gender === 'unisex' && styles.selectedGenderChip]}
              onPress={() => setGender('unisex')}
            >
              <Text style={[styles.genderChipText, gender === 'unisex' && styles.selectedGenderText]}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            style={[styles.input, { textAlign: isArabic ? 'right' : 'left' }]}
            placeholder={isArabic ? 'مثال: 56-58 سم' : 'e.g. 56-58 cm'}
            value={customSize}
